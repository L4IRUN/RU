    ...
    sortedMonths.forEach(month => {
        ...
        groupedNotes[month].forEach(date => {
            ...
            if (typeof noteData === 'string') {
                ...
                if (noteData.morning) contentText += `${noteData.morning} `;
                if (noteData.afternoon) contentText += `${noteData.afternoon} `;
                if (noteData.evening) contentText += `${noteData.evening}`;
                timeText = noteData.timestamp || timeText;
            }
...
function updateNoteDOM(date) {
    ...
    if (existingCard) {
        ...
        let contentText = '';
        const n = notes[date];
        if (n.morning) contentText += `${n.morning} `;
        if (n.afternoon) contentText += `${n.afternoon} `;
        if (n.evening) contentText += `${n.evening}`;
